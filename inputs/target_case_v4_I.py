...
DEFAULT_PEAK = {
    "unit": "nm",
    "fwhm_eV": 0.2,
    "fwhm_angstrom": None,   # Angstrom; uniform wavelength window across all peaks
    "fwhm_nm": None,
    "sigma_eV": None,
    "type": "anchor",
}
...
CONFIGURATIONS = [
    {
        ...
        "optimize_radial_base": True,   # survey winner: ground_plus_4d9_nl
...
SEARCH = {
    ...
    "scoring": {
        ...
        # UTA centroid mode: group_window = search_window so all lines in the
        # window form one group whose center is the gA-weighted UTA centroid.
        # This matches the physical observation better than picking a single
        # dominant sub-cluster, which can be offset by 1-4 A.
        "local_shift_search_window_angstrom": 1.5,
        "local_line_group_window_angstrom": 1.5,
        "max_local_shift_groups_per_peak": 1,
    },
}
...
#     max(shift_p) - min(shift_p) <= max_shift_spread_nm.
# For example, shifts of +0.30 nm and +0.26 nm pass with a 0.05 nm spread limit.
GLOBAL_SHIFT = {
    "enabled": True,
    "mode": "per_peak_consistency",
    "require_consistent_direction": False,
    "direction_zero_tolerance_angstrom": 1.0e-6,
    "max_shift_spread_nm": 1.0,
...
def _normalize_configuration(entry, index, used_ids):
    ...
    # "{n}l" / "{m}k" (e.g. "5l", "6k") expand all valid angular momenta
    # for a single fixed principal quantum number.
